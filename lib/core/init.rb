...
module ProjectmasterofcloudDependencyStack
  extend ActiveSupport::Concern
  include Rubycfn

  included do
    description "masterofcloud#{ENV["ENVIRONMENT"].capitalize}Dependency Stack"
...
stack = include ProjectmasterofcloudDependencyStack # rubocop:disable Style/MixinUsage
...
previous_statuses = []
80.times do
  previous_events = get_prior_events(client, "masterofcloud#{ENV["ENVIRONMENT"].capitalize}DependencyStack")
  ...
  break if last_event \
    && (last_event.logical_resource_id == "masterofcloud#{ENV["ENVIRONMENT"].capitalize}DependencyStack") \
    && (last_event.stack_name == "masterofcloud#{ENV["ENVIRONMENT"].capitalize}DependencyStack") \
...
if stack_exists
  client.update_stack(
    stack_name: "masterofcloud#{ENV["ENVIRONMENT"].capitalize}DependencyStack",
...
360.times do
  ...
  resp.stack_events.to_a.reverse.each_with_index do |event, index|
    ...
    if (@stack_name == "masterofcloud#{ENV["ENVIRONMENT"].capitalize}DependencyStack") \
      && (@logical_resource_id == "masterofcloud#{ENV["ENVIRONMENT"].capitalize}DependencyStack") \
